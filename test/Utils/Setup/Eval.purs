...
import Halogen.Hooks.Internal.Eval.Types (HookState(..), HalogenM')
...
import Test.Utils.Setup.Types (DriverResultState, HalogenF', LogRef, TestEvent(..))
...
evalM ref (H.HalogenM hm) = Aff.Driver.Eval.evalM mempty ref (foldFree go hm)
  ...
  go = case _ of
    ...
    H.Subscribe fes k -> do
      sid <- liftAff $ fresh SubscriptionId ref
      finalize <- liftEffect $ HS.subscribe (fes sid) \act ->
        Aff.Driver.Eval.handleAff $ evalF ref (Input.Action act)
      -- Aff.Driver.Eval.handleAff $ Aff.Driver.Eval.handleLifecycle 
...
evalF
  :: forall r q b
   . Ref (DriverResultState r q b)
  -> Input.Input (HookM Aff Unit)
  -> Aff Unit
evalF ref = case _ of
  Input.Action act -> do
    DriverState st <- liftEffect (Ref.read ref)
    evalM ref $ st.component.eval (HQ.Action act unit)
  _ -> do
    pure unit
...
fresh
  :: forall r s f act ps i o a
   . (Int -> a)
  -> Ref.Ref (DriverState r s f act ps i o)
  -> Aff a
fresh f ref = do
  DriverState st <- liftEffect (Ref.read ref)
